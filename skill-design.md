...
flowchart TD
    ...
    Sel --> Align[③ 确定对齐锚点<br/>对象 / 理由 / 锚点类型]
...
- **2026-08-16 v119**:真实内容驱动补入 P1/P2 两张关系版式。P1「黑箱剖面 + 编号标注」归流动、上下不对称，不抽组件；P2「菱形四边标注」归映射、单区，抽为仅浏览 `new:11 diamond-edge-labels`，不进入 production routing-manifest。P2 的四节点菱形补齐左下第 4 条实体边，页面帧与 Catalog 抄录件同步。Catalog 关系版式 54→56、组件 71→72，gallery-manifest 配方 62→64；缩略图门禁页面 +2、组件 +1。边界:wise-ppt 机器内核下游 registry 仍须在 kernel 侧由生成器重算，禁止在这里手填连锁指纹。

- **2026-08-16 v120**:封面四向分化 + 居中纹理模板。D1 保留左题右 UI 图；D7 改为上方 Agent 工程栈、下方居中标题；D8 改为左工作台、右侧长标题；新增 D10 居中标题 + 确定性波纹纸纤维纹理。封面 1→4、非关系页模板 8→11、整页帧 64→67，缩略图门禁页面 81→84、总映射 153→156。边界:D7/D8/D10 是非关系页模板，不进入关系配方或组件路由。
