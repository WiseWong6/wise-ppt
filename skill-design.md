...
7. **可用高度内垂直居中**(v80 立,v81 精确化):可用高度上缘 = 眉题(.doc)底部;下缘分两种 —— **有底句(题注)页取底句顶部,无底句页(封面/金句/宣言/尾页)取页码(.folio)顶部**。组件按内容包围盒(排除全部家具)垂直居中于该区域;**必须浏览器实测**——逐页量 bbox 与上下缘,|内容中线 − 区域中线| ≤ 3px 才算过,不许用估的常数。
