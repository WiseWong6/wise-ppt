...
flowchart TD
    ...
    PType -->|关系页| Relation[① 定关系<br/>七大族 22 种选一]
...
A | B      无关系,只是放一起
```
加上"只有 1 个对象""对齐比较""定量数据展示"三种边界情况,穷尽为 **7 大族**。
...
多个数值,主角是数字本身(排名/分布/指标)? → 数据 Data
```

7 个分支,**互斥、穷尽**。排行、分布、指标直接进数据族,不再冒充对比/陈列/空间。
...
| ⑦ 数据 Data | 排名 ranking | 按数值大小排序 |
| | 分布 distribution | 数值按位置/区域展开(地图) |
| | 指标 metric | 多个关键数字并排 |
...
6. **数值页先进数据族** —— 排名/分布/指标是数据展示,不冒充对比/陈列/空间;示意页(插图/原型)归重心,不冒充证据
...
| 结构 | 说明 |
|------|------|
| 单行 | 整页一块,组件形状自由(放射/环形/蛇形/漏斗/矩阵…) |
| 左右x等分 | 横向均分,x≥2 |
| 上下x等分 | 纵向均分,x≥2 |
| 左右不对称 | 主+次,次侧可再分 |
| 上下不对称 | 主+次(纵向),三种走法:上主下收束带 / 上主下辅×N / 上辅下主 |
...
> **矩阵(表格)是组件画法,不是页面切分。** 三栏行带矩阵 → 左右x等分(K4,三栏是切分,行带是栏内画法);整页矩阵 → 单行(F1 象限);矩阵 + 收束带/KPI 通栏 → 上下不对称(E4、K1)。与放射/环形/蛇形/漏斗同级:结构只认 6 种矩形分块,不设「表格」。

> 已删除冗余项:左右等分/上下等分(= x等分 x=2)、象限(= 网格 2×2)、上下N段(K1/E4 用"上下不对称"覆盖)、表格(= 组件画法,按页面切分归单行/左右x等分/上下不对称)。

> **边界规则:题注带不算切分。** 页面顶部/底部仅一行题注/出处(约占高度 15% 以内)是大图的附属说明,不构成独立内容区,归单行(A2 场景画)。

> 可视化浏览:`references/catalog.html?tab=str`(结构枚举 tab)。每种切分一节:抽象示意图(切分变体)+ 判断要点 + 承载关系 + 归入版式全部 iframe 实拍。55 个关系页版式按结构字段程序化归入:单行 25 / 左右x等分 11 / 上下x等分 5 / 左右不对称 7 / 上下不对称 5 / 网格 2。
...
|---|---|---|
| 焦点 focus | 单行 | — |
| 示意 illustration | 单行(插图/原型展示) | A2、A8 |
| 陈列 | 左右x等分 / 上下x等分 / 网格mxn | A5、A7、F4 |
| 并行 parallel | 左右x等分 / 单行(泳道图,时间轴贯穿) | I4、B3 |
| 层级 hierarchy | 单行(树/同心环) / 上下x等分(分层栈) / 左右不对称(树+规格) | H3、F2 |
| 拆解 decomposition | 单行(放射) / 左右x等分 / 网格 / 上下x等分 / 左右不对称 / 上下不对称(上总额下四环) | G1、F3、F4、H4、F2、C5 |
| 部分整体 part-whole | 同拆解 | — |
| 嵌套 nesting | 单行(同心环/嵌套框) | H1、H2 |
| 时序 sequence | 单行(时间轴/阶梯/蜿蜒) / 左右x等分(步骤条) | B1、B5、B6、A9 |
| 流动 flow | 单行(流水线) / 上下x等分(双轨) | I1、I3 |
| 循环 cycle | 单行(圆环/蛇形);若带辅助内容 → 左右不对称 | J1、J2 |
| 汇聚 convergence | 单行(汇聚图) | N1 |
| 漏斗 funnel | 单行(漏斗图) / 左右不对称(漏斗 + 两侧标注) | O1 |
| 因果 causal | 上下x等分(因上果下) / 左右x等分(痛→解→效) / 左右不对称(左痛右解) | E3、K2、K3 |
| 对比 comparison | 左右x等分 / 上下x等分(前后双带) | E1、E2、E6 |
| 矩阵 matrix | 左右x等分(三栏行带矩阵) / 单行(整页矩阵) / 上下不对称(矩阵+收束带) | K4、F1、K1、E4 |
| 映射 mapping | 单行(弧线网) / 左右x等分(双列映射) | L2、E5 |
| 网络 network | 单行(节点连线) | L1、L2 |
| 证据 evidence | 单行(证据墙) / 左右不对称(策略+证据) / 上下不对称(摘要+原文) | A1、A6、A4、A3 |
| 排名 ranking | 单行(排行柱图) | C7 |
...
- 浏览: `references/catalog.html`(三个 tab:非关系页模板 / 关系页版式按七大族 / 组件按组合并)
