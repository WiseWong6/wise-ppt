...
> **预览遗产打穿(v88,物化必做清单)**:组件库 CSS 是按 600px 方卡预览写的,直接注入会缩在槽中间一小块。必须用槽级覆写逐项打穿,汇总进 `registered-components.css` 尾部:
...
- **2026-08-16 v94**:Catalog #063 `radial-progress` 恢复 C5 血统。v25 把它压成“纯四环水平单元×N”,丢掉了 C5「上总量 + 下四项拆解」的核心表达,因此卡片虽有环形指标却已不是原构图。修复:Native 证据 SVG 在 950×440 frame 内补回上区总量主项、四环英文副标与下区共享合计基线,继续保留 v77 已删除的英文装饰标题/两端圆点不恢复;Catalog 标签从 `单元×N / 陈列` 改为 `源 C5 · 上总量+下方环列阵 / 拆解`。生产路由 ID、1–4 data-point 容量和 C5 `topology.split-y-2.dominant-end` 上下不对称版式合同不变。
