...
> 可视化浏览:`references/catalog.html?tab=str`(结构枚举 tab)。每种切分一节:抽象示意图(切分变体)+ 判断要点 + 承载关系 + 归入版式(点击跳版式 tab)+ 代表版式实拍。55 个关系页版式按结构字段程序化归入:单行 23 / 左右x等分 11 / 上下x等分 7 / 左右不对称 7 / 上下不对称 3 / 网格 2 / 表格 2(E4"上下2等分+底带"按行列双语义计表格)。
