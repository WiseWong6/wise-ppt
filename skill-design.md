...
> 可视化浏览:`references/catalog.html?tab=str`(结构枚举 tab)。每种切分一节:抽象示意图(切分变体)+ 判断要点 + 承载关系 + 归入版式全部 iframe 实拍。55 个关系页版式按结构字段程序化归入:单行 24 / 左右x等分 11 / 上下x等分 6 / 左右不对称 8 / 上下不对称 2 / 网格 2 / 表格 2(E4"上下2等分+底带"按行列双语义计表格)。
