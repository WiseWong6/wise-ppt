...
本文件是 experimental 的唯一 Markdown 权威。experimental 只做一件事：在已交付 standard 的隔离副本中，重画获批页面的结构、组件和视觉；内容合同与原主题不变。

## 授权

standard 已依次减字、换骨架、拆页，仍需重新组织结构或核心组件时，先停止制作并提交申请：
...
批准页内部可用 HTML/CSS/SVG 自由重做结构、组件和视觉。新增样式必须写在带 `data-experimental-page-style="<page-id>"` 的 style 中，所有选择器都限定到该页，并只使用原主题变量。此权限只属于用户明确批准的 experimental，不得进入 standard。
