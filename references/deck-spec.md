...
`example` 保留图册原文，只说明文字槽的内容职责，不保证原文符合新内容的填写上限；不能按示例长度推断容量，也不能照抄原文。公开 `max_chars` 按去除空白后的 Unicode 字符计数（中英文、数字和标点各算一个），是锁定字体和内部空间下的保守字符上限。预检和构建使用同一规则检查 `text/data/claim`，实际排版仍须通过浏览器 fit 检查；它不允许作者缩小字号、压间距、改 CSS/DOM 或绕过浏览器 fit 门禁。
...
- 左上（support 槽）两行：第一行 `deck.title`，第二行是概括本页内容的**页面标题**，写在现有 payload 槽中，不新增字段。关系页的页面标题不得照抄底部 `claim`；关系页 `support.text.001/002` 各一行，非关系页单键换行分两行，违反即编译失败。
- 底部（关系页 caption）是本页希望观众接受的**核心论点**，由 `claim` 填写。例如页面标题为“Rubric 评分标准的建立”，核心论点为“没有 rubric 的模型评分，会把不确定性转交给另一个模型”。
