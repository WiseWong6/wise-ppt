...
发现疑似不匹配时，不修改 standard。先列出 `page_id`、当前 `layout_id`、不匹配证据、已经尝试过的减字/换骨架/拆页，以及建议实验的准确页面范围；按 [主说明的授权规则](../SKILL.md#先决定模式) 核对，已有该范围有效授权时按实验合同继续，否则询问用户是否进入 experimental。未经明确批准不继续；没有发现时回报“未发现语义—版式/组件不匹配”。这一步默认不截图，也不替代下面的人工视觉验收。
