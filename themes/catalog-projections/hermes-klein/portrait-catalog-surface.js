(function () {
  ...
  function legacyForegroundMembers() {
    if (root.dataset.emphasisProfile !== 'legacy') return [];
    try {
      var foreground = JSON.parse(root.dataset.catalogLegacyEmphasisForegroundMembers || '[]');
      return Array.isArray(foreground) ? foreground : [];
    } catch (error) {
      throw new Error('旧强调前景成员不是合法 JSON: ' + error.message);
    }
  }

  function applyLegacyForeground(stage) {
    stage.querySelectorAll(
      '[data-xp-focus-foreground],[data-xp-focus-foreground-neutral],[data-xp-focus-foreground-active]'
    ).forEach(function (node) {
      node.removeAttribute('data-xp-focus-foreground');
      node.removeAttribute('data-xp-focus-foreground-neutral');
      node.removeAttribute('data-xp-focus-foreground-active');
    });
    var count = 0;
    legacyForegroundMembers().forEach(function (member, index) {
      if (!member || typeof member.selector !== 'string' || !member.selector) {
        throw new Error('legacy foreground_members[' + index + '] 缺 selector');
      }
      if (!Number.isInteger(member.expected_count) || member.expected_count < 1) {
        throw new Error('legacy foreground_members[' + index + '] expected_count 必须是正整数');
      }
      var nodes;
      try {
        nodes = Array.prototype.slice.call(stage.querySelectorAll(member.selector));
      } catch (error) {
        throw new Error('legacy foreground_members[' + index + '] selector 非法: ' + error.message);
      }
      if (nodes.length !== member.expected_count) {
        throw new Error(
          'legacy foreground_members[' + index + '] 数量不符: ' + member.selector
          + ' expected=' + member.expected_count + ' actual=' + nodes.length
        );
      }
      nodes.forEach(function (node) {
        node.dataset.xpFocusForeground = 'true';
        if (member.neutral_tone && member.neutral_tone !== 'preserve') {
          node.dataset.xpFocusForegroundNeutral = member.neutral_tone;
        }
        if (member.active_tone && member.active_tone !== 'preserve') {
          node.dataset.xpFocusForegroundActive = member.active_tone;
        }
        count += 1;
      });
    });
    stage.dataset.xpFocusForegroundCount = String(count);
  }
  ...
  function resetFocusRetry() {
    if (focusRetryFrame) cancelAnimationFrame(focusRetryFrame);
    focusRetryFrame = 0;
    focusRetryCount = 0;
    delete root.dataset.xpFocusState;
  }

  function deferFocusMismatch() {
    /* Catalog 会把相邻 iframe 停泊到 display:none 的缓存里。动态批注在隐藏态
       没有可用几何，恢复可见后会自行重画并再次调用 apply。A1 等动态 SVG
       在字体或状态重绘时也会短暂清空节点，最多等待与绘制器相同的八帧。 */
    if (!stageIsVisible()) {
      root.dataset.xpFocusState = 'hidden-deferred';
      return true;
    }
    if (focusRetryCount >= 8) return false;
    root.dataset.xpFocusState = 'members-pending';
    root.dataset.catalogSurfaceReady = 'false';
    if (!focusRetryFrame) {
      focusRetryFrame = requestAnimationFrame(function () {
        focusRetryFrame = 0;
        focusRetryCount += 1;
        apply();
      });
  ...
  function canvasTokens(spec, palette) {
    ...
    /* FUNCTIONAL 是纸墨母版自带的主题通道；比例不能改变它的语义。
       3:4 中误用该通道的对象应在页面源里修正，不能在运行时把整条通道洗成结构墨。 */
  ...
  function apply() {
    ...
    var focusNodeSets = [];
    var focusMismatch = null;
    focusMembers.forEach(function (member, index) {
      if (!member || !member.selector ||
          (!legacyEmphasis && (typeof member.treatment !== 'string' || member.treatment.indexOf('focus.') !== 0))) {
        throw new Error('focus_members[' + index + '] 缺 selector/treatment');
      }
      if (!Number.isInteger(member.expected_count) || member.expected_count < 1) {
        throw new Error('focus_members[' + index + '] expected_count 必须是正整数');
      }
      var nodes;
      try {
      ...
      focusNodeSets[index] = nodes;
      if (!focusMismatch && nodes.length !== member.expected_count) focusMismatch = member;
    });
    if (focusMismatch) {
      if (deferFocusMismatch()) return;
      throw new Error('强调成员数量不符: ' + focusMismatch.selector);
    }
    resetFocusRetry();
