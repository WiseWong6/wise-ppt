(function initSwissEChartsAdapter(root, factory) {
  const api = factory('swiss.echarts', 'swiss');
  ...
  /* 与 paper-ink adapter 同一套 token 驱动的视觉规则；Swiss 的差异全部落在
     design-tokens.css（白底、中性灰 ramp、accent 模式安全橙 ramp），此处不写死色值。 */
