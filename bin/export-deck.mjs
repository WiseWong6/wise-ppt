...
`),manifest}finally{cdp.close()}}const experimentalFurnitureScript=String.raw`(() => Array.from(document.querySelectorAll('#track > .slide')).map(slide => {
  const origin = slide.getBoundingClientRect();
  return { page_id: slide.dataset.pageId, nodes: Array.from(slide.querySelectorAll('.doc.tl,.folio,.caption')).flatMap(root =>
    [root, ...root.querySelectorAll('*')].map(node => {
      const rect = node.getBoundingClientRect();
      const style = getComputedStyle(node);
      return {
        geometry: [rect.left - origin.left, rect.top - origin.top, rect.width, rect.height],
        style: ['fontFamily','fontSize','fontWeight','lineHeight','letterSpacing','color','display','visibility','opacity']
          .map(key => style[key]),
      };
    })) };
}))()`;async function loadExperimentalPrintPage(cdp,url){await cdp.send("Page.navigate",{url}),await waitUntilReady(cdp),await cdp.evaluate(`(() => {
    document.documentElement.classList.add('print-mode');
    document.body.classList.remove('mode-board');
    document.body.classList.add('mode-deck');
  })()`),await settle(cdp)}async function exportExperimentalDeck({deckDir,url,port,pdfPath}){const sourceHtml=await readFile(path.join(deckDir,"index.html"),"utf8"),sourceSlideCount=countSourceSlides(sourceHtml);sourceSlideCount<1&&fail("实验 index.html 中没有 section.slide[data-page-id]");const cdp=await connectCdp(port);try{const browserVersion=await cdp.send("Browser.getVersion");await cdp.send("Emulation.setDeviceMetricsOverride",{width:1920,height:1080,deviceScaleFactor:1,mobile:!1}),await cdp.send("Emulation.setEmulatedMedia",{media:"print"});const workspace=JSON.parse(await readFile(path.join(deckDir,".wise-ppt-experiment"),"utf8"));await loadExperimentalPrintPage(cdp,pathToFileURL(path.join(workspace.source.path,"index.html")).href);const baselineFurniture=await cdp.evaluate(experimentalFurnitureScript);await loadExperimentalPrintPage(cdp,url);const currentFurniture=await cdp.evaluate(experimentalFurnitureScript);for(const[index,baseline]of baselineFurniture.entries()){const current=currentFurniture[index];(!current||current.page_id!==baseline.page_id||current.nodes.length!==baseline.nodes.length||current.nodes.some((node,i)=>JSON.stringify(node.style)!==JSON.stringify(baseline.nodes[i].style)||node.geometry.some((value,j)=>Math.abs(value-baseline.nodes[i].geometry[j])>1)))&&fail(`实验页 ${baseline.page_id} 固定家具的浏览器位置或样式偏离 standard`)}const state=await cdp.evaluate(String.raw`(() => {
      const slides = Array.from(document.querySelectorAll('#track > .slide'));
      const pages = slides.map((slide, index) => {
        ...
        while (walker.nextNode()) {
          ...
          if (!text.textContent.trim() || !visibleElement(text.parentElement)) continue;
          const range = document.createRange();
          range.selectNodeContents(text);
          if (!Array.from(range.getClientRects()).some(rect => rect.width > 0 && rect.height > 0)) continue;
          textNodes.push(text.parentElement);
          paintedText.push(text.textContent);
        }
        const visibleRuns = [' ', ''].map(separator => paintedText.join(separator).replace(/\s+/g, ' '));
        const missingRequiredVisible = requiredVisible.filter(required => (
          typeof required !== 'string' || !visibleRuns.some(text => text.includes(required.replace(/\s+/g, ' ')))
        ));
        const smallText = textNodes.filter(node => !node.closest('.doc.tl,.folio,.caption'))
          .map((node) => ({
