...
`doctor` 只检查、不下载；失败就停止。不得要求安装 Python、lxml、Poppler、`pdfinfo`、Homebrew、`curl` 或 npm 依赖。已有同家族同字重字体就复用；缺失字体由首次 `build` 自动下载，下载与缓存做 SHA-256 校验。

用 Chrome 打开离线 `references/catalog.html`；成品范例见 `themes/paper-ink/examples/wise-ppt-story-six-page/`。
...
`standard`：只输入 `deck-spec.json`，只用登记结构和公开槽；逐页强调进入最终 HTML/PDF。生成物无手改，三道命令与 manifest 全通过。
